...
struct month_day {
   inline month_day( alaio::month m, alaio::day d ) : m(m), d(d) {}
...
constexpr inline uint64_t char_to_name_digit_strict() {
   static_assert(is_valid_char(C), "character is not for an alaio name");
...
constexpr inline uint64_t string_to_name_strict_impl() {
   if constexpr (N == 12)
      static_assert((char_to_name_digit_strict<C>() & 0xf) == char_to_name_digit_strict<C>(),
            "alaio name 13th character cannot be a letter after j");
...
constexpr inline uint64_t string_to_name_strict() {
   static_assert(sizeof...(Str) <= 13, "alaio name string is too long");
...
[[nodiscard]] inline bool string_to_symbol(uint64_t& result, uint8_t precision, const char*& pos, const char* end,
                                           bool require_end) {
   if (!alaio::string_to_symbol_code(result, pos, end, require_end))
      return false;
...
inline std::string symbol_to_string(uint64_t v) {
   return std::to_string(v & 0xff) + "," + alaio::symbol_code_to_string(v >> 8);
}
...
[[nodiscard]] inline bool string_to_asset(int64_t& amount, uint64_t& symbol, const char*& s, const char* end,
   ...
   amount = uamount;
   uint64_t code;
   if (!alaio::string_to_symbol_code(code, s, end, expect_end))
      return false;
...
inline std::string asset_to_string(int64_t amount, uint64_t symbol) {
   ...
   return result + ' ' + alaio::symbol_code_to_string(symbol >> 8);
}
