...
class symbol_code {
 public:
   ...
   constexpr explicit symbol_code(std::string_view str) : value(0) {
      if (str.size() > 7) {
         alaio::check(false, "string is too long to be a valid symbol_code");
      }
      for (auto itr = str.rbegin(); itr != str.rend(); ++itr) {
         if (*itr < 'A' || *itr > 'Z') {
            alaio::check(false, "only uppercase letters allowed in symbol_code string");
         }
         value <<= 8;
         value |= *itr;
      }
...
ALAIO_REFLECT(symbol_code, value);
ALAIO_COMPARE(symbol_code);
...
ALAIO_REFLECT(symbol, value);
ALAIO_COMPARE(symbol);
...
ALAIO_REFLECT(extended_symbol, sym, contract);
ALAIO_COMPARE(extended_symbol);
} // namespace alaio
