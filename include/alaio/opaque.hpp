...
class opaque_base {
 ...
 public:
    ...
    * @pre !this->empty()
    */
   [[deprecated("Use unpack() free function instead.")]] void unpack(T& obj) { alaio::from_bin(obj, bin); }
   ...
   template <typename S>
   void from(S& stream) {
      alaio::from_bin(this->bin, stream);
   }

   template <typename S>
   void to_bin(S& stream) const {
      alaio::to_bin(this->bin, stream);
   }
