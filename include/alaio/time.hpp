...
ALAIO_REFLECT(microseconds, _count);
ALAIO_COMPARE(microseconds);
...
ALAIO_REFLECT(time_point, elapsed);
ALAIO_COMPARE(time_point);

template <typename S>
void from_json(time_point& obj, S& stream) {
   auto s = stream.get_string();
   uint64_t utc_microseconds;
   if (!alaio::string_to_utc_microseconds(utc_microseconds, s.data(), s.data() + s.size())) {
      check(false, convert_json_error(alaio::from_json_error::expected_time_point));
   }
...
void to_json(const time_point& obj, S& stream) {
   return to_json(alaio::microseconds_to_str(obj.elapsed._count), stream);
}
...
class time_point_sec {
 public:
   ...
   static time_point_sec min() { return time_point_sec(0); }
            ...
            operator time_point() const { return time_point(alaio::seconds(utc_seconds)); }
   uint32_t sec_since_epoch() const { return utc_seconds; }

   /// @cond INTERNAL
   time_point_sec operator=(const alaio::time_point& t) {
...
ALAIO_REFLECT(time_point_sec, utc_seconds);
ALAIO_COMPARE(time_point);
...
void from_json(time_point_sec& obj, S& stream) {
   auto s = stream.get_string();
   const char* p = s.data();
   if (!alaio::string_to_utc_seconds(obj.utc_seconds, p, s.data() + s.size(), true, true)) {
...
void to_json(const time_point_sec& obj, S& stream) {
   return to_json(alaio::microseconds_to_str(uint64_t(obj.utc_seconds) * 1'000'000), stream);
}
...
class block_timestamp {
 public:
   ...
   block_timestamp next() const {
      alaio::check(std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow");
...
ALAIO_REFLECT(block_timestamp_type, slot);
