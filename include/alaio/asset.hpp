...
struct asset {
   ...
   asset(int64_t a, class symbol s) : amount(a), symbol{ s } {
      alaio::check(is_amount_within_range(), "magnitude of asset amount must be less than 2^62");
      alaio::check(symbol.is_valid(), "invalid symbol name");
   }
   ...
   asset& operator-=(const asset& a) {
      alaio::check(a.symbol == symbol, "attempt to subtract asset with different symbol");
      amount -= a.amount;
      alaio::check(-max_amount <= amount, "subtraction underflow");
      alaio::check(amount <= max_amount, "subtraction overflow");
      return *this;
   }
    ...
    */
   asset& operator+=(const asset& a) {
      alaio::check(a.symbol == symbol, "attempt to add asset with different symbol");
      amount += a.amount;
      alaio::check(-max_amount <= amount, "addition underflow");
      alaio::check(amount <= max_amount, "addition overflow");
      return *this;
   }
   ...
   asset& operator*=(int64_t a) {
      ...
      alaio::check(tmp >= -max_amount, "multiplication underflow");
      amount = (int64_t)tmp;
      return *this;
   }
#endif
    ...
    */
   asset& operator/=(int64_t a) {
      alaio::check(a != 0, "divide by zero");
      alaio::check(!(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow");
      amount /= a;
      return *this;
   }
   ...
   friend int64_t operator/(const asset& a, const asset& b) {
      alaio::check(b.amount != 0, "divide by zero");
      alaio::check(a.symbol == b.symbol, "comparison of assets with different symbols is not allowed");
...
ALAIO_REFLECT(asset, amount, symbol);

template <typename S>
inline void from_string(asset& result, S& stream) {
   int64_t  amount;
   uint64_t sym;
   check(alaio::string_to_asset(amount, sym, stream.pos, stream.end, true),
      convert_stream_error(alaio::stream_error::invalid_asset_format));
...
struct extended_asset {
   ...
   friend extended_asset operator-(const extended_asset& a, const extended_asset& b) {
      alaio::check(a.contract == b.contract, "type mismatch");
...
ALAIO_REFLECT(extended_asset, quantity, contract);
} // namespace alaio
