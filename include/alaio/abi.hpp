...
constexpr inline std::string_view convert_abi_error(alaio::abi_error e) {
   switch (e) {
...
ALAIO_REFLECT(type_def, new_type_name, type);
...
ALAIO_REFLECT(field_def, name, type);
...
ALAIO_REFLECT(struct_def, name, base, fields);
...
ALAIO_REFLECT(action_def, name, type, ricardian_contract);
...
ALAIO_REFLECT(table_def, name, index_type, key_names, key_types, type);
...
ALAIO_REFLECT(clause_pair, id, body);
...
ALAIO_REFLECT(error_message, error_code, error_msg);
...
ALAIO_REFLECT(variant_def, name, types);
...
ALAIO_REFLECT(action_result_def, name, result_type);
...
ALAIO_REFLECT(primary_key_index_def, name, type);

struct secondary_index_def {
   std::string type;
};

ALAIO_REFLECT(secondary_index_def, type);
...
ALAIO_REFLECT(abi_def, version, types, structs, actions, tables, ricardian_clauses, error_messages, abi_extensions,
...
struct abi {
   std::map<alaio::name, std::string> action_types;
   std::map<alaio::name, std::string> table_types;
   std::map<std::string, abi_type>    abi_types;
   std::map<alaio::name, std::string> action_result_types;
