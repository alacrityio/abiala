...
abi_type* get_type(std::map<std::string, abi_type>& abi_types,
   ...
   alaio::check(depth < 32,
        alaio::convert_abi_error(abi_error::recursion_limit_reached));
   ...
   alaio::check(depth < 32,
    ...
    if (!type->base.empty()) {
        ...
        if(auto* b = std::get_if<abi_type::struct_>(&base->_data)) {
           ...
           alaio::check(false, alaio::convert_abi_error(abi_error::base_not_a_struct));
        }
    }
...
abi_type::alias resolve(std::map<std::string, abi_type>& abi_types, const abi_type::alias_def* type, int depth) {
    ...
    alaio::check(!std::holds_alternative<abi_type::extension>(t->_data),
        alaio::convert_abi_error(abi_error::extension_typedef));
...
const abi_type* alaio::abi::get_type(const std::string& name) {
   return ::get_type(abi_types, name, 0);
}

void alaio::convert(const abi_def& abi, alaio::abi& c) {
    ...
    for (auto& t : abi.types) {
       alaio::check(!t.new_type_name.empty(),
...
void to_abi_def(abi_def& def, const std::string& name, const T*) {
   alaio::check(false, alaio::convert_abi_error(alaio::abi_error::bad_abi));
}
...
void alaio::convert(const alaio::abi& abi, alaio::abi_def& def) {
   def.version = "alaio::abi/1.0";
